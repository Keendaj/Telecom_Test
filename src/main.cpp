...
std::vector<u8> parseAdjStrict(const std::string& s) {
    ...
    for (char c : s) {
        if (!std::isdigit(c) && c != ',') {
            throw std::invalid_argument("Forbidden character in adjacency list");
        }
    }
...
int main() {
    ...
    for (int i = 0; i <= n; ++i) {
        ...
        try {
            ...
            lab.addRoom(id, adj, static_cast<u8>(iron), static_cast<u8>(gold), static_cast<u8>(gems), static_cast<u8>(exp));
    ...
    for (const auto& [u, neighbors] : adjacency_map) {
        for (u8 v : neighbors) {
            ...
            if (it == adjacency_map.end() || 
                std::find(it->second.begin(), it->second.end(), u) == it->second.end()) {
