...
struct BotAction {
    ...
    u8 target_id = 0;                             ///< ID целевой комнаты (используется только при type == MOVE).
    ResourceType resource = ResourceType::NONE;   ///< Тип собираемого ресурса (используется только при type == COLLECT).
};
...
public:
     ...
     * @param current_room Данные о комнате, в которой бот находится прямо сейчас.
     * @param current_food Текущий запас еды .
     * @return Действие (BotAction), которое бот хочет совершить.
     */
    virtual BotAction getNextAction(const Room& current_room, u8 current_food) = 0;
};