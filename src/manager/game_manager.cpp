...
GameManager::GameManager(Labyrinth &labyrinth, IBot &bot, u8 initial_food)
    : _labyrinth(labyrinth), _bot(bot), _current_food(initial_food),
      _current_room_id(0) {}
...
void GameManager::logState(std::ostream &out, u8 room_id) {
    ...
    out << "state " << static_cast<int>(room_id) << " "
        << (res.iron_collected ? "_"
                               : std::to_string(static_cast<int>(res.iron)))
        << " "
        << (res.gold_collected ? "_"
                               : std::to_string(static_cast<int>(res.gold)))
        << " "
        << (res.gems_collected ? "_"
                               : std::to_string(static_cast<int>(res.gems)))
        << " "
...
SimulationResult GameManager::run(std::ostream &out, ResourceType target) {
    ...
    while (true) {
        ...
        if (action.type == ActionType::FINISH) {
            ...
            if (result.success) {
                u32 total_value = ScoreCalculator::calculateTotalValue(
                    result.collected_resources, target);
                out << "result "
        ...
        if (action.type == ActionType::MOVE) {
            ...
            bool is_free = (free_collect_used.find(_current_room_id) ==
                            free_collect_used.end());
            ...
            u8 collected_amount =
                _labyrinth.collectResource(_current_room_id, action.resource);
